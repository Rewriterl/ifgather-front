<template>
  <div class="container">
    <Breadcrumb :items="['menu.scan', 'menu.scan.webinfo']" />
          ...
          >
            <a-row>
              ...
              <a-col :span="12">
                <a-form-item
                  field="searchParams"
                  :label="$t('searchTable.columns.banalyze')"
                >
                  <a-input
                    v-model="formModel.searchParams.banalyze"
                    :placeholder="$t('searchTable.columns.banalyze')"
                  />
                </a-form-item>
              </a-col>
            </a-row>
...
<script lang="ts" setup>
  ...
  import { getWebinfoList } from '@/api/webinfo';
  ...
  const columns = computed<TableColumnData[]>(() => [
    ...
    {
      title: t('searchTable.columns.content_length'),
      dataIndex: 'content_length',
    },
    {
      title: t('searchTable.columns.banalyze'),
      dataIndex: 'banalyze',
    },
    {
  ...
  const fetchData = async (
    ...
    setLoading(true);
    try {
      const data: any = await getWebinfoList(params);
